...
- **Delivery** — target branch (main; hotfixes fix-forward via `fix/*`) · PR after build (no · yes → `/be-ship`)
...
## 2. Run the build pipeline (each step is an existing skill — consult be-skill-index first)

1. **`/be-schema-complete`** — be-schema-generator → be-sql-design-guard → be-db-migration-maintainer → be-rls-tenant-isolation-guard.
2. **be-domain-generator** — repository → service → controller → dto/validator/serializer/types → container + route registration.
3. **`/be-route-complete`** — be-api-contract-guard → be-route-schema-doc-guard → be-route-catalog → be-seed-maintainer (+ be-openapi-multilingual if tags).
4. **be-workers-events** — only if the intake declares events/queues/workers.
5. **be-seed-maintainer** — reference + bulk/faker rows per the intake.
6. **be-test-generator** (+ **be-contract-test-maintainer** / **be-chaos-test-maintainer** when section 7 marks them) — generate the test layers declared in section 7 (unit, integration, e2e, smoke; contract/chaos if needed); the intake's cases become the acceptance tests.
7. **be-i18n-message-guard**, **be-tsdoc-export-guard**, **be-overview-doc-maintainer**, OpenAPI — i18n + docs.

## 3. Autonomous verify → heal loop

Run the gates; on failure, route long output through `headroom_compress`, diagnose, fix, and re-run — until green. **Definition of done:** `pnpm validate` + the route/domain gates + a live `pnpm verify:base` smoke + **`/be-pre-merge-review`** clean. Escalate (`AskUserQuestion`) **only** on ambiguity, an irreversible/destructive step, a security trade-off, or repeated no-progress failure.
...
- **review.md** — the `/be-pre-merge-review` output.
- **quality.md** — test/tsdoc/route coverage + lint/type status; security notes (RLS/tenant, idempotency, secrets).

Commit per step so the build is resumable. Do **not** open a PR unless asked — when the slice is green, suggest `/be-ship`.
