...
4. **rls-tenant-isolation-guard** — tenant-owned tables `ENABLE` + `FORCE` RLS with an organization-scoped policy carrying both `USING` and `WITH CHECK`; the `app.current_organization_public_id` GUC set on every query path.
