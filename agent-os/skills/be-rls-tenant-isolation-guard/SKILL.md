---
name: be-rls-tenant-isolation-guard
...
- [ ] Tenant-scoped jobs carry `organizationPublicId` in the payload (typed `OrganizationScopedJobData`); user-scoped jobs carry `userPublicId`.
...
- [ ] **Keep the scope families as separate types — never merge `PrincipalDatabaseScope` and `SessionDatabaseScope` or make GUC keys field-driven.** Values are dynamic (the context arms exactly what the scope carries); keys are static per family (bypass GUCs have no path through the app wrapper). The types carry different minting-confinement locks — merging them collapses the allowlists. The wrapper merge (`withAppDatabaseContext`) was the safe half and is done; worked LEGAL/ILLEGAL examples: rls-architecture.md §3.
- [ ] **Keep the callback shape — never refactor `with*DatabaseContext` to an open/commit API or `await using`.** The callback is the price of "release is impossible to forget, commit/rollback is automatic, and nesting reuses the transaction" (`AsyncLocalStorage.run` needs the function span; `enterWith` leaks). Nesting is one level deep in practice and reads as a scoped block — if indentation annoys, extract the body into a named function, don't change the wrapper.
...
```sql
BEGIN;
SELECT set_config('app.current_organization_public_id','A',true);
SAVEPOINT s; SELECT set_config('app.current_organization_public_id','B',true); RELEASE SAVEPOINT s;
SELECT current_setting('app.current_organization_public_id');   -- 'B', not 'A'
```

**Rule:** inside a nested transaction, set **every** GUC the RLS identity depends on, on **every** iteration — never only the one that row happens to need. A branch that sets just its own GUC inherits the other branch's value from the previous row, silently widening the policy for the remainder of the batch. Reference: the per-row savepoint in `audit-outbox-drain.processor.ts` writes both `app.current_organization_public_id` and `app.system_audit_insert` unconditionally for exactly this reason.
...
Related: [[be-workers-events]] (queue/worker patterns), [[be-sql-design-guard]] (schema design), [[be-db-migration-maintainer]] (migration authoring).

---

**Related skills:** [be-schema-generator](../be-schema-generator/SKILL.md) · [be-db-migration-maintainer](../be-db-migration-maintainer/SKILL.md) · [be-workers-events](../be-workers-events/SKILL.md)
