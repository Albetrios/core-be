...
- **rls-tenant-isolation-guard** — the scope types and what each GUC grants; consult it when choosing the key's scope.
- **tsdoc-export-guard** — every cache export needs a summary and `@remarks`.
- **change-completeness-guard** — the cache's docs (`PATTERNS.md`, `read-caching.md`, `POLICIES.md`, the cache table in the reference doc) move with the code.
