...
indexNote: reference only
---

# Cursor global skills (reference)

These skills ship with **Cursor** under `~/.cursor/skills-cursor/`. They are **not** part of the core-be repo. Use **be-skill-index** and `agent-os/skills/*` for backend work in this project.
...
| Cursor IDE settings / extensions.json              | **update-cursor-settings** (user); **be-ide-productivity-guard** (project `.vscode/`) |
...
| **create-rule**            | Author `agent-os/rules/*.mdc`                               |
...
| **be-split-to-prs**           | Split work into small reviewable PRs                       |
...
**45 total** — 42 actionable skills plus **be-skill-index** (meta) and **be-cursor-global-skills** (this reference). **be-lint-warnings-handler** is a detail skill invoked via **be-code-smells-and-best-practices**, not counted separately.
