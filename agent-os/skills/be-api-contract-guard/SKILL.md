...
> brevity is a deliberate wire-size choice. The be-full-names-only rule applies to
...
`detail` is localized human copy that changes with wording/locale (see **be-i18n-message-guard**).
...
**Searching a column that lives on a FORCE-RLS joined table** (e.g. member email/name in `auth.users`) can't use a plain join under organization-only context — it needs a SECURITY DEFINER resolver. See `be-rls-tenant-isolation-guard` (member-search example, `tenancy.search_organization_membership_ids`).
...
**Related skills:** [be-route-schema-doc-guard](../be-route-schema-doc-guard/SKILL.md) · [be-route-catalog](../be-route-catalog/SKILL.md) · [be-seed-maintainer](../be-seed-maintainer/SKILL.md) · [be-idempotency-guard](../be-idempotency-guard/SKILL.md)
