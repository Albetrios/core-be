...
| Network isolation in contexts | `src/tests/global/rls-context-network-isolation.global.test.ts` | No outbound I/O (Stripe/S3/Resend/fetch) inside `withAppDatabaseContext` / `withAppDatabaseContext` / `withMaintenanceDatabaseContext` / `withTransaction` callbacks. |
...
| Scope shape contract | `src/tests/unit/infrastructure/database/scope-shape-contract.policy.unit.test.ts` | Every minter emits exactly its family's fixed keys — Principal `{ userPublicId?, organizationPublicId?, source }`, Session `{ kind, value }`, Maintenance `{ kind }` (frozen) — so no doorway can grow, drop, or rename a field silently. |
