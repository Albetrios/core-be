...
| 14 | short forms in prose/identifiers (`org`, `orgs`, `orgB`, `org-permission`) | `organization` everywhere — prose, test titles, catalog data, script names | full-names rule extended beyond identifiers; INTENTIONAL compact wire literals stay: `org_` prefix, JWT claims `org`/`sv` (documented in be-full-names-only.mdc + be-api-contract-guard) |
