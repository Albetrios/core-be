...
| .env variable      | Where to get it                                  |
| ------------------ | ------------------------------------------------ |
| `SCALAR_API_KEY`   | Scalar API key.                                  |
| `SCALAR_NAMESPACE` | Your Scalar team namespace.                      |
| `SCALAR_SLUG`      | Registry slug (optional; defaults to `core-be`). |

Use a **distinct `SCALAR_SLUG` per environment** (e.g. `core-be-dev` vs `core-be`) on each GitHub Environment so hosted docs do not overwrite each other.

**Steps:** [Scalar Dashboard](https://dashboard.scalar.com) → **Settings** → **API Keys** → create → copy key. Note your **team namespace** from the dashboard or registry URL.
