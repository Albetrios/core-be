...
describe('Security: worker RLS database context', () => {
  ...
  it('allows tenant-scoped reads when wrapped in withOrganizationDatabaseContext and createWorkerSubscriptionRepository', async () => {
    process.env.CORE_BE_RUNTIME = 'worker';
    const owner = await createTestUser();
    const organization = await createTestOrganization({ ownerUserId: owner.id });

    await withOrganizationDatabaseContext(organization.public_id, async (databaseHandle) => {
