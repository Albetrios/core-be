...
| **core-fe journey** | `scenarios/fe-user-journey.js` | credential pool + API on `TEST_MODE=true` with `TEST_STATIC_VERIFICATION_CODE` | `VUS=50 POOL=50 k6 run src/tests/load/k6/scenarios/fe-user-journey.js` — the full front-end journey, one pass per VU (VUs are users). Knobs: `VUS`, `POOL`, `AUTH` (`code`\|`password`\|`otp`), `STATIC_CODE`, `STAGGER`, `RESULT_TAG` |
