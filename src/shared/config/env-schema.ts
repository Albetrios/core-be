...
export const envSchema = envSchemaBase
  ...
  .refine(
    ...
    {
      // The fallback in google-oauth.provider.ts builds `${FRONTEND_URL}/callback` (the SPA's
      // provider-agnostic OAuth landing page, which forwards code+state to this API's callback
      // route). The value must match the provider-console registration exactly, so require it
      // whenever the provider is configured — a loud boot failure beats a silently wrong redirect
      // that only surfaces as a provider-side `redirect_uri_mismatch`.
      message:
        "OAUTH_GOOGLE_REDIRECT_URI is required when OAUTH_GOOGLE_CLIENT_ID is set — it must match the Google Cloud Console entry exactly and point at the SPA's /callback page (e.g. https://app.example.com/callback).",
  ...
  .refine(
    ...
    {
      message:
        "OAUTH_GITHUB_REDIRECT_URI is required when OAUTH_GITHUB_CLIENT_ID is set — it must match the GitHub App callback URL exactly and point at the SPA's /callback page (e.g. https://app.example.com/callback).",
