...
- **Workers must use `runOrganizationScopedWorkerJob` / `runGlobalRetentionWorkerJob` / `runUserScopedWorkerJob`** from [worker-runtime/worker-processor.util.ts](src/infrastructure/queue/worker-runtime/worker-processor.util.ts). Workers are forbidden from importing `request-database.context.ts` (enforced by the worker-context guard in [contexts/database-context-runtime.ts](src/infrastructure/database/contexts/database-context-runtime.ts) and the worker-database-guard static-scan test).
