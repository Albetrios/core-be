...
const FORCE_RLS_ALLOWED_KINDS: ReadonlySet<WorkerDatabaseContextKind> = new Set([
  ...
  // it resolves under per-row organization or app.global_admin context. RLS still scopes every row — listing
...
export function assertWorkerDatabaseContext(
  ...
  if (context === undefined) {
    throw new WorkerDatabaseContextError(
      'Worker process must not use unpinned database access. Wrap the job in a context helper (withAppDatabaseContext, runOrganizationScopedWorkerJob, withMaintenanceDatabaseContext, or withAppDatabaseContext) and pass databaseHandle into createWorker*Repository() factories.',
    );
  }
 ...
 * worker wrappers so RLS-bound jobs carry the organization identity through ALS.
 */
