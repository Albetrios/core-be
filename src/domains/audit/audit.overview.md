...
- **Two read paths, two RLS contexts**: org listing runs in `withPrincipalDatabaseContext` (org scope, `app.current_organization_id`); global admin listing runs under `MAINTENANCE_SCOPE.global_admin` (`app.global_admin = true`) so cross-tenant reads are explicit under FORCE RLS, not table-owner bypass.
