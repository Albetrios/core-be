...
- **Outbox-staging RLS context** (audit R10): `AuditService.record` stages the row in `audit.outbox`, whose INSERT policy (`audit_outbox_tenant_isolation_insert`) has only two arms — an organization arm (`organization_public_id = app.current_organization_public_id`) and a tenantless system arm (`organization_public_id IS NULL AND app.system_audit_insert = 'true'`); there is **no** user arm. So `record` opens the matching context per row: organization-scoped rows under an organization principal scope, tenantless rows (login, auth, admin, queue-dashboard) under `MAINTENANCE_SCOPE.SYSTEM_AUDIT_INSERT`. Without this the bare-pool INSERT is rejected under the production `core_be_app` role (FORCE/ENABLE RLS) and the row is silently dropped by `recordAuditEvent`. The staged row is its own short transaction — decoupled from the originating business write (which already committed in its own scoped context post-sec-M4) and best-effort.
- **Two read paths, two RLS contexts**: organization listing runs in `withAppDatabaseContext` (organization scope, `app.current_organization_public_id`); global admin listing runs under `MAINTENANCE_SCOPE.GLOBAL_ADMIN` (`app.global_admin = true`) so cross-tenant reads are explicit under FORCE RLS, not table-owner bypass.
...
- **Per-route cap**: `GET /api/v1/audit/logs` carries `MODERATE_AUTHED_RATE_LIMIT` (30 req/60s keyed by user). The preset's `preHandler` hook is appended after `requireRole`, so a denied caller is rejected before its bucket key is derived and cannot consume an admin's quota. Pinned by `audit-routes-rate-limit.policy.unit.test.ts`.
...
- `tenant-isolation` / `rls-context` — outbox-staging writes run under the organization principal context for organization-scoped rows or `MAINTENANCE_SCOPE.SYSTEM_AUDIT_INSERT` for tenantless rows, matching the two arms of the `audit.outbox` INSERT policy.
