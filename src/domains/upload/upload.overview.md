...
- **Reservation RLS context matches the row** (sec-r7/M4): an org-scoped upload reserves its `pending` row under `withPrincipalDatabaseContext` with an org scope (sets `app.current_organization_public_id`) so the `uploads_tenant_isolation` `WITH CHECK` is satisfied; a personal upload (`organization_id IS NULL`) reserves under `withPrincipalDatabaseContext` with a user scope for `uploads_owner_access`. Reserving an org row under user context is rejected by RLS as the production `core_be_app` role (FORCE RLS) — verified in `src/tests/security/rls/upload-rls.security.test.ts`.
- **Owner-scoped**: personal uploads (`organization_id IS NULL`) are owned by the presigning user. Org-scoped uploads require current `upload:manage` on get/confirm/delete — former members cannot mutate org assets via the user-owned upload routes.
