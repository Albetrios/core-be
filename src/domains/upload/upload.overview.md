...
- **Magic-byte verification on confirm**: after HEAD, confirm reads the object's leading bytes and rejects mismatches against the declared `content_type` (closing the spoofed-content-type vector where S3 only echoes the client-declared type).
...
- **Reservation RLS context matches the row** (sec-r7/M4): an organization-scoped upload reserves its `pending` row under `withAppDatabaseContext` with an organization scope (sets `app.current_organization_public_id`) so the `uploads_tenant_isolation` `WITH CHECK` is satisfied; a personal upload (`organization_id IS NULL`) reserves under `withAppDatabaseContext` with a user scope for `uploads_owner_access`. Reserving an organization row under user context is rejected by RLS as the production `core_be_app` role (FORCE RLS) — verified in `src/tests/security/rls/upload-rls.security.test.ts`.
- **Owner-scoped**: personal uploads (`organization_id IS NULL`) are owned by the presigning user. Org-scoped uploads require current `upload:manage` on get/confirm/delete — former members cannot mutate organization assets via the user-owned upload routes.
