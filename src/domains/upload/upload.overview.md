...
- **Reservation RLS context matches the row** (sec-r7/M4): an org-scoped upload reserves its `pending` row under `withAppDatabaseContext` with an org scope (sets `app.current_organization_public_id`) so the `uploads_tenant_isolation` `WITH CHECK` is satisfied; a personal upload (`organization_id IS NULL`) reserves under `withAppDatabaseContext` with a user scope for `uploads_owner_access`. Reserving an org row under user context is rejected by RLS as the production `core_be_app` role (FORCE RLS) — verified in `src/tests/security/rls/upload-rls.security.test.ts`.
