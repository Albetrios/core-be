...
/**
 * Plan #15 — BullMQ job stores stripeEventId only; worker fetches full event for processing.
 */
describe('stripe-webhook — id-only Redis payload (payload shrink)', () => {
  ...
  it('processes a job whose Redis payload contains only stripeEventId', async () => {
    const stripeEventId = `evt_payload_shrink_${randomUUID()}`;
    const eventPayload = {
      id: stripeEventId,
    ...
    const jobId = `stripe-payload-shrink-${randomUUID()}`;
    const completion = waitForJobCompletion(queueEvents!, jobId);

    await queue!.add('process-stripe-webhook', { stripeEventId }, { jobId, attempts: 1 });
