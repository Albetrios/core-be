...
>) {
  return {
    ...
    getMe: async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      const data = await userService.getMe(requireUserPrincipalDatabaseScope(request));
    ...
    completeOnboardingMe: async (request: FastifyRequest, _reply: FastifyReply) => {
      const _auth = requireAuth(request);
      const data = await userService.completeOnboarding(requireUserPrincipalDatabaseScope(request));
      return successResponse(data, getRequestIdentifier(request));
    },
    ...
    getSettings: async (request: FastifyRequest, _reply: FastifyReply) => {
      const _auth = requireAuth(request);
      const data = await userSettingsService.get(requireUserPrincipalDatabaseScope(request));
      return successResponse(data, getRequestIdentifier(request));
    },

    patchSettings: async (request: FastifyRequest, _reply: FastifyReply) => {
      const auth = requireAuth(request);
      const data = await userSettingsService.update(
        requireUserPrincipalDatabaseScope(request),
        request.body,
      );
      await recordScopedAuditEvent(request, {
    ...
    getNotificationPreferences: async (request: FastifyRequest, _reply: FastifyReply) => {
      const _auth = requireAuth(request);
      const data = await userNotificationPreferencesService.get(
        requireUserPrincipalDatabaseScope(request),
      );
      return successResponse(data, getRequestIdentifier(request));
    },
