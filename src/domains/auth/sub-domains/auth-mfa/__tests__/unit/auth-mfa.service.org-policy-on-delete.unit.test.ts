 ...
 * password-only authentication in direct contradiction of organization policy. Removing any
...
describe('MfaService.deleteMfa — organization MFA policy guard (sec-A4)', () => {
  ...
  it('refuses to delete the LAST MFA method when the user belongs to an MFA-required org', async () => {
    ...
    // The organization-policy guard fires AFTER the candidate-method lookup but BEFORE the revoke;
  ...
  it('allows deleting the last MFA method when no organization of the user requires MFA', async () => {
  ...
  it('allows deleting a non-last MFA method even when organization policy requires MFA (another factor remains)', async () => {
