...
- **Explicit invalidation on every permission write, AFTER commit** (audit R11): any change to a user's role / membership / role-permission assignment invalidates the `(user, org)` cache key once the write transaction has committed — never inside the `withPrincipalDatabaseContext` block. Invalidating pre-commit races a concurrent recompute that reads the old committed set and re-caches it before the write lands, so a downgraded/removed member would keep access until TTL. Enforced by `src/tests/unit/api/permission-cache-post-commit.policy.unit.test.ts`.
- **Org-wide invalidation via a version key**: a change that can affect many users (role-permission set replaced, role deleted, org soft-deleted) bumps `perm:org:<org>:v` with a single `INCR` (+`EXPIRE`, audit-#T3) instead of a SCAN sweep; every cached key is scoped by that version, so the bump instantly orphans them all. The recompute lock's commit binds to the version **captured before the recompute's DB read** (audit-#H1) — if a concurrent org-wide bump lands mid-recompute, the stale commit writes into the now-orphaned old namespace instead of re-publishing revoked permissions under the live version.
- **Fail-closed version reads**: a corrupt/errored org-version read throws rather than silently returning `0` (which would delete the wrong `perm:0:*` key); the per-user invalidator surfaces the failure to Sentry and bumps the org version as a backstop so a stale entry cannot survive to its TTL (audit-#T0/#T1).
